...
describe Article do
  ...
  let(:article) {
    Article.new({ "title" => title, "url" => url, "description" => description })
  }
  ...
  describe ".all" do
    it "should return an array of Article objects corresponding to the database data" do
      first_article_data = ["Valid Title 1", "https://www.google.com", "Valid description text 1"]
      last_article_data = ["Valid Title 2", "https://www.amazon.com", "Valid description text 2"]
  ...
  describe "#valid?" do
    ...
    context "for an invalid object" do
      let(:article_with_blank_attributes) { Article.new({ "title" => "", "url" => "", "description" => "" }) }
      let(:article_with_invalid_url) { Article.new({ "title" => title, "url" => "invalid url", "description" => description }) }
      let(:article_with_too_short_description) { Article.new({ "title" => title, "url" => url, "description" => "hi mom"}) }
      let(:article_with_invalid_attributes) { Article.new({ "title" => "", "url" => "invalid url", "description" => "hi mom"}) }
      ...
      it "should add an error message if there is an article with the same url in the database" do
        db_connection do |conn|
          sql_query = %(
          INSERT INTO articles (title, url, description)
          VALUES ($1, $2, $3)
      ...
      it "should add an error message if the description is too short" do
        ...
        expect(article_with_too_short_description.errors).to eq([description_too_short_message])
      end
